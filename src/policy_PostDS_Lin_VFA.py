...
class policy_PostDS_Lin_VFA(Pol):
    ...
    def __init__(self, Name, FilePath, GLB_VARS, params):
        ...
        def f_1(state):
            return self.Nodes['R_1'].get_postds_value(state)
        def f_2(state):
            return self.Nodes['E'].get_postds_value(state)
        def f_3(state):
            return self.Nodes['G'].get_postds_value(state)
    ...
    def decision(self, state):
        ...
        for i in xrange(len(Feasible_Decisions)):
            ...
            TempPostDS['T']=T
            BFs=apply_basis_functions(state,self.Basis_Functions)
            Val=beta_hat[len(beta_hat)-1]
            for bf in xrange(len(BFs)):
                Val+=(beta_hat[bf]*BFs[bf])    
            DecVal=self.Reward(state,Feasible_Decisions[i],self.Nodes)+self.Discount_Factor*Val
