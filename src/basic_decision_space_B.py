...
class basic_decision_space_B(decision_space):
                    ...
                    if (dv[0][0]=='G' and dv[1][0]=='L') or (dv[1][0]=='G' and dv[0][0]=='L'):
                        ...
                        if (dv[1][0]=='L'):
                            if (dv[0][0]=='E'):
                                ...
                                DP1=np.arange(0.0,min([Curr_val,Curr_val_2])+d_int, d_int)
                                DP2=np.linspace(0.0,min([Curr_val,Curr_val_2]),num=self.num_poss_per_decision_var[dv[0]+':'+dv[1]])
                                if len(DP2)>len(DP1):
                                ...
                                Curr_val_2=self.energy_vars[dv[0]].get_value(current_state)
                                d_int=self.energy_vars[dv[0]].get_discretization_interval()
                                DP1=np.arange(0,min([Curr_val,Curr_val_2])+d_int, d_int)
                                DP2=np.linspace(0,min([Curr_val,Curr_val_2]),num=self.num_poss_per_decision_var[dv[0]+':'+dv[1]])
                                if len(DP2)>len(DP1):
                                    Decision_Possibilities[dv[0]+':'+dv[1]]=DP1
                                else:
                                    Decision_Possibilities[dv[0]+':'+dv[1]]=DP2
