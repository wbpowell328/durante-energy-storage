...
class Example_Decision_Space(decision_space):
    
    def __init__(self, GLOBAL_VARS):
        
        self.glb_vars=GLOBAL_VARS
        ...
        self.Nodes=GLOBAL_VARS.get_global_variable('Nodes')
    ...
    def allowed_actions(self, current_state):
        ...
        '''
        From the current state, return a list of feasible decisions.
        '''

        E_curr=self.Nodes['E'].get_preds_value(current_state)
        L_curr=self.Nodes['L'].get_preds_value(current_state)
        R_curr=self.Nodes['R_1'].get_preds_value(current_state)
        Decision_Possibilities={}
        Res1=self.Nodes['R_1']
        d_int=Res1.get_discretization_interval()
        R_max=Res1.get_max()
        CR=Res1.max_charge_rate
        Eff=Res1.conv_rate
        ...
        for D in Decisions_1:
            ...
            for GR1 in set(GRlist):
                DD=copy(D)
                DD['G:R_1']=GR1
                decisions_forward.append(DD)
        return decisions_forward
